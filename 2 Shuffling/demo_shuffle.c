...
int main() {
    ...
    if (numbers_check == 1)
        printf(
            "numbers check PASSED: x is in shuffled if and only if x is in "
            "unshuffled.\n");
    else
    ...
    if (greek_check == 1)
        printf(
            "greek check PASSED: x is in shuffled if and only if x is in "
            "unshuffled.\n");
    else
...
int check_shuffle(void* L, int len, int size, int (*cmp)(void*, void*)) {
    ...
    free(shuffled);
    /* By this point, every element in the shuffled set exists in the unshuffled
     * set and vice-versa. Therefore the shuffled set and unshuffled set are
     * equal. */
    return 1;
}
